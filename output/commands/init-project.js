...
const handlebars_1 = require("../utils/handlebars");
...
async function initProject(_installpath, options) {
    try {
        let installpath = _installpath;
        if (options?.startMode) {
            const cwd = path_1.default.resolve(process.cwd(), installpath);
            const currentDirectory = await (0, app_finder_1.findTSDIAPIServerProject)(cwd);
            if (currentDirectory) {
        ...
        let projectDir = (0, cwd_1.isPathSuitableToNewProject)(installpath);
        if (!projectDir) {
            return process.exit(1);
        }
        const dirAvailable = (0, cwd_1.isDirSuitableToNewProject)(installpath);
        if (!dirAvailable) {
            ...
            try {
                const { newDir } = await inquirer_1.default.prompt([{
                        ...
                        validate: async (input) => {
                            return (0, cwd_1.isDirSuitableToNewProject)(input) ? true : "❌ Directory is not empty or invalid.";
                        }
                    }]);
                installpath = newDir;
                console.log(chalk_1.default.yellow(`📁 Using new directory: ${installpath}`));
                projectDir = path_1.default.resolve(process.cwd(), newDir);
            }
            catch (error) {
                console.error(chalk_1.default.red("❌ An unexpected error occurred during project initialization."), error.message);
                process.exit(1);
...
async function installation(projectDir, options) {
    ...
    try {
        ...
        for (const env of envFiles) {
            ...
            const envContent = (0, handlebars_1.buildHandlebarsTemplate)("env", { ...payload, isProduction: env.isProduction });
            if (envContent) {
        ...
        const packageContent = (0, handlebars_1.buildHandlebarsTemplate)("package", payload);
        if (packageContent) {
...
`;
        ...
        const homePageContent = (0, handlebars_1.buildHandlebarsTemplate)("home", payload);
        if (homePageContent) {
        ...
        const appConfigContent = (0, handlebars_1.buildHandlebarsTemplate)("app_config", payload);
        if (appConfigContent) {
        ...
        const mainContent = (0, handlebars_1.buildHandlebarsTemplate)("main", payload);
        if (mainContent) {
